          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "4rem" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem", marginBottom: "3rem" }}>
                {[
                  { icon: <Mail size={18} color="var(--orange)" />, label: "Email", value: COMPANY.email, href: `mailto:${COMPANY.email}` },
                  { icon: <Phone size={18} color="var(--orange)" />, label: "Nigeria", value: COMPANY.phone2, href: `tel:${COMPANY.phone2}` },
                  { icon: <Phone size={18} color="var(--orange)" />, label: "Canada", value: COMPANY.phone1, href: `tel:${COMPANY.phone1}` },
                  { icon: <Globe size={18} color="var(--orange)" />, label: "Website", value: COMPANY.website, href: `https://${COMPANY.website}` },
                ].map((item) => (
                  <a key={item.label} href={item.href} target={item.label === "Website" ? "_blank" : undefined} rel="noopener noreferrer" style={{ display: "flex", gap: "1rem", alignItems: "flex-start", background: "var(--bg-card)", border: "1px solid var(--border-green)", padding: "1.25rem", borderRadius: "3px", transition: "border-color 0.2s ease", textDecoration: "none" }}>
            ...
            {/* Contact person + enquiry types */}
            <div>
              <span className="section-eyebrow">Primary Contact</span>
              <span className="accent-line" />
              ...
              <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-green)", borderTop: "3px solid var(--orange)", padding: "2rem", borderRadius: "3px", marginBottom: "2rem" }}>
                <div style={{ display: "flex", gap: "1.25rem", alignItems: "flex-start", marginBottom: "1.5rem" }}>
                  <div style={{ width: "64px", height: "64px", background: "linear-gradient(135deg, var(--green-dark), var(--green))", border: "2px solid var(--orange)", borderRadius: "4px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontFamily: "var(--font-display)", fontSize: "1.4rem", color: "#fff" }}>NU</div>
                  <div>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: "1.3rem", color: "var(--text-primary)", letterSpacing: "0.03em" }}>Nsikak (Nsi) Udott</div>
                    <div style={{ fontFamily: "var(--font-condensed)", fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--orange)", marginBottom: "0.4rem" }}>Executive Director</div>
                    <span className="badge badge-green">P.Eng. MBA</span>
                  </div>
                </div>
                ...
                <p style={{ fontSize: "0.87rem", color: "var(--text-muted)", lineHeight: 1.7, margin: "0 0 1.25rem" }}>
                  Nsi is the primary contact for all investor relations, partnership enquiries and project discussions. He is a Registered Professional Engineer (APEGA) with 15+ years of experience in energy and industrial project delivery across Canada and Nigeria.
                </p>
      ...
      <section style={{ background: "linear-gradient(135deg, var(--green-dark), var(--bg-navbar))", borderTop: "2px solid var(--orange)", padding: "4rem 0" }}>
        <div className="container" style={{ textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem", justifyContent: "center" }}>
            ...
            <Link href="/financials" className="btn-secondary">Financial Model <ArrowRight size={16} /></Link>
          </div>
        </div>
      </section>
